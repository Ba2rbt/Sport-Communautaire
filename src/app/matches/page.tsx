...
export default async function MatchesPage() {
  ...
  const matchesByDate = matchesData.reduce((acc, match) => {
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold font-sans flex items-center gap-3">
              Match Center
              {liveMatchesCount > 0 && <span className="px-2 py-0.5 bg-red-500/20 text-red-500 text-xs rounded animate-pulse border border-red-500/50">LIVE</span>}
            </h1>
             ...
             {[...Array(7)].map((_, i) => {
               ...
               return (
                 <button 
                  ...
                  `}
                >
                   <span className="text-xs uppercase">{d.toLocaleDateString('fr-FR', { weekday: 'short' }).replace('.', '')}</span>
                            ...
                            <div className="relative overflow-hidden rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all duration-300">
                                ...
                                <div className="relative p-6 flex items-center gap-6">
                                    <div className="w-16 flex flex-col items-center gap-1 border-r border-white/5 pr-6">
                                        <span className={`text-sm font-bold ${match.status === 'live' ? 'text-red-500' : 'text-slate-400'}`}>
