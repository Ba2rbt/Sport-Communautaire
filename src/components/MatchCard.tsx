    ...
    >
      ...
      <div className="relative p-5 sm:p-6 z-10">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {match.isLive ? (
            <BadgeLive size="sm" className="bg-red-500/20 text-red-500 border border-red-500/20 shadow-lg shadow-red-500/10" />
          ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between group/team p-2 rounded-lg hover:bg-white/5 transition-colors -mx-2">
            <TeamLink teamName={match.homeTeam.name} className="flex items-center gap-4 min-w-0 flex-1">
            ...
            {match.status !== 'upcoming' && (
              <span className={`text-2xl font-bold font-mono min-w-[2rem] text-right ${(match.homeScore ?? 0) > (match.awayScore ?? 0) ? 'text-accent-live' : 'text-slate-400'}`}>
                {match.homeScore ?? 0}
              </span>
            )}
          </div>
          ...
          {/* Away Team */}
          <div className="flex items-center justify-between group/team p-2 rounded-lg hover:bg-white/5 transition-colors -mx-2">
            <TeamLink teamName={match.awayTeam.name} className="flex items-center gap-4 min-w-0 flex-1">
              <div className="relative">
                <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center text-2xl shadow-inner border border-white/5 group-hover/team:border-white/10 transition-colors">
                  {match.awayTeam.logo}
                </div>
            ...
            {match.status !== 'upcoming' && (
              <span className={`text-2xl font-bold font-mono min-w-[2rem] text-right ${(match.awayScore ?? 0) > (match.homeScore ?? 0) ? 'text-accent-live' : 'text-slate-400'}`}>
                {match.awayScore ?? 0}
              </span>
            )}
          </div>
